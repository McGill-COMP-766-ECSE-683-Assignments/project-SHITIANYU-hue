...
class ReplayBuffer(object):
	...
	def load(self, save_folder, size=-1):
		...
		self.state[:self.size] = np.load(f"{save_folder}_state.npy",allow_pickle=True)[:self.size]
		self.action[:self.size] = np.load(f"{save_folder}_action.npy")[:self.size]
		self.next_state[:self.size] = np.load(f"{save_folder}_next_state.npy",allow_pickle=True)[:self.size]
		self.reward[:self.size] = reward_buffer[:self.size].reshape(-1,1)
		self.not_done[:self.size] = np.load(f"{save_folder}_not_done.npy")[:self.size].reshape(-1,1)
		self.goal[:self.size] = np.load(f"{save_folder}_goal.npy")[:self.size]