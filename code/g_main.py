...
def train_BCQ(state_dim, action_dim, max_action, device, args):
	...
	while training_iters < args.max_timesteps: 
		risk = policy.train(replay_buffer, iterations=int(args.eval_freq), batch_size=args.batch_size)
		risks.extend(risk)
		evaluations.append(eval_policy(policy, args.env, args.seed))
		# np.save(f"./results/BCQnoise_{setting}", evaluations)
		# np.save(f"./results/risknoise_{setting}",risks)
...
def eval_policy(policy, env_name, seed, eval_episodes=10):
	...
	for _ in range(eval_episodes):
		...
		while not done:
			action = policy.select_action(np.array(state['observation']),np.array(state['desired_goal']))
			state, reward, done, _ = eval_env.step(action)
			# env.render()
			print(_.get('is_success', False))
			# print(action)
			avg_reward += reward
...
if __name__ == "__main__":
	...
	parser.add_argument("--env", default="parking-v0")               # OpenAI gym environment name
	...
	parser.add_argument("--start_timesteps", default=25e2, type=int)# Time steps initial random policy is used before training behavioral
	...
	parser.add_argument("--render", action="store_true")   # If true, render env
